...
plus the planned three-member rollout matrix, is tracked in
[`A2A_MIGRATION_SDK_IT_SCENARIOS.md`](A2A_MIGRATION_SDK_IT_SCENARIOS.md).
Verified standalone rows are included below; remaining cluster rows stay
outside current coverage claims until their executable tests run successfully.
...
The default and Jackson 3 adapters discover the same 101-test matrix. On
